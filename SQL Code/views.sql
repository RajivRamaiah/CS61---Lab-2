...
WHERE MANUSCRIPT.NUMBER=REVIEWER_GROUP.MANUSCRIPT_NUMBER AND REVIEWER_GROUP.REVIEWER_NUMBER=4 GROUP BY MANUSCRIPT.STATUS;
...
SELECT MANUSCRIPT.STATUS as Status, COUNT(*) as Count FROM MANUSCRIPT WHERE MANUSCRIPT.AUTHOR_ID=1 GROUP BY MANUSCRIPT.STATUS;

SELECT * FROM MANUSCRIPT WHERE MANUSCRIPT.AUTHOR_ID=1 GROUP BY MANUSCRIPT.STATUS;


SELECT * FROM MANUSCRIPT NATURAL JOIN REVIEWER_GROUP WHERE MANUSCRIPT.NUMBER=REVIEWER_GROUP.MANUSCRIPT_NUMBER;




SELECT REVIEW.REVIEWER_NUMBER as Reviewer, REVIEW.MANUSCRIPT_NUMBER as Num FROM REVIEW WHERE REVIEW.MANUSCRIPT_NUMBER=4 AND REVIEW.REVIEWER_NUMBER=2;



INSERT INTO CREDENTIALS VALUE ("AUTHOR", 5, AES_ENCRYPT('a','AA12345678'));


SELECT AES_DECRYPT(CREDENTIALS.PASSWORD, 'AA12345678') as PASSWORD FROM CREDENTIALS WHERE CREDENTIALS.USER_TYPE='Author' AND CREDENTIALS.ID=5;



SELECT REVIEWER_GROUP.MANUSCRIPT_NUMBER as ManNum, REVIEWER_GROUP.REVIEWER_NUMBER as RevNum FROM REVIEWER_GROUP 
WHERE REVIEWER_GROUP.MANUSCRIPT_NUMBER=1 AND REVIEWER_GROUP.REVIEWER_NUMBER=6;



SELECT MANUSCRIPT.STATUS as Status, MANUSCRIPT.NUMBER as Num FROM MANUSCRIPT WHERE MANUSCRIPT.AUTHOR_ID=2;


SELECT MANUSCRIPT.STATUS as Status, COUNT(*) as Count FROM MANUSCRIPT WHERE MANUSCRIPT.AUTHOR_ID=" + id +  " GROUP BY MANUSCRIPT.STATUS ORDER BY MANUSCRIPT.STATUS;


SELECT MANUSCRIPT.NUMBER as ManuscriptNumber, MANUSCRIPT.STATUS as Status FROM MANUSCRIPT NATURAL JOIN REVIEWER_GROUP
WHERE MANUSCRIPT.NUMBER=REVIEWER_GROUP.MANUSCRIPT_NUMBER AND REVIEWER_GROUP.REVIEWER_NUMBER=2 ORDER BY MANUSCRIPT.STATUS;




SELECT MANUSCRIPT.NUMBER as ManuscriptNumber, MANUSCRIPT.STATUS as Status FROM MANUSCRIPT WHERE MANUSCRIPT.AUTHOR_ID=3 ORDER BY FIELD(MANUSCRIPT.STATUS, 'Received', 'Under Review', 'Rejected', 'Accepted', 'Typeset', 'Scheduled', 'Published');


SELECT MANUSCRIPT.NUMBER as Num FROM MANUSCRIPT WHERE MANUSCRIPT.JOURNAL_ISSUE_YEAR=2007 AND MANUSCRIPT.JOURNAL_ISSUE_PERIOD=4;
